...
This project is positioned as a portfolio and practice system for Exem/Exemble-adjacent LLM platform work. The primary signal is not a general-purpose chatbot UI; it is a production-style RAG/Agent backend that demonstrates document ingestion, retrieval quality, evidence-aware answering, observability, and cloud deployment.
...
| LLM Providers | OpenRouter `nvidia/nemotron-3-super-120b-a12b:free`, OpenAI-compatible, Anthropic | Baseline model plus pluggable adapters |
...
- Baseline model: `nvidia/nemotron-3-super-120b-a12b:free` through OpenRouter.
- Paid routing is blocked with OpenRouter `max_price` input/output set to `0`.
- The application does not use runtime fallback chains; model changes are explicit `LLM_MODEL` environment changes.
- RAG quality is evaluated through deterministic golden-set metrics rather than claims about free-model quality.
...
- `잊어줘:` — Delete memory
- `대화 요약해줘` — Generate conversation summary
...
- Tool choice combines an LLM structured decision with deterministic guardrails for document, web, summary, and report intents
...
- Primary goal: portfolio-grade Production-level RAG/Agent backend for Exem/Exemble-adjacent LLM platform practice.
- RAG success criteria: grounded answers, abstention when evidence is missing, citation/evidence visibility, and deterministic eval.
- Document pipeline scope: structured parsing, Markdown/table preservation, hierarchy-aware parent-child retrieval.
- Agentic workflow scope: single `AssistantAgent` with tool-using workflow, not a multi-agent router.
- Model strategy: fixed free NVIDIA OpenRouter baseline with no runtime fallback chain.
...
5. Add configurable paid-model profiles for higher reliability when cost constraints are relaxed
