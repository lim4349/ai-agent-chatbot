...
async def record_agent_metrics(
    ...
    Usage:
        async with record_agent_metrics(store, sid, "assistant", llm.config.model) as metrics:
...
def extract_token_usage_from_response(response) -> tuple[int, int]:
    ...
    # Dict responses (structured output) never carry usage metadata — skip warning
    if input_tokens == 0 and output_tokens == 0 and not isinstance(response, dict):
        logger.warning(
